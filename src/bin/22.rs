    ...
    input.lines()
        ...
        .flat_map(|(i, l)| {
            ...
            l.bytes()
                .enumerate()
                .filter(|(_, b)| *b != b' ')
                .map(move |(j, b)| ((ni as isize, j as isize), b == b'#'))
        }).collect::<HashMap<_,_>>()
}

fn parse_instr(input: &str) -> (Vec<u32>, Vec<usize>) {
    ...
    for i in 1..input.len() {
        if bi[i] == b'L' || bi[i] == b'R' {
            ...
            movs.push(num);
            rots.push(if bi[i] == b'L' { 3 } else { 1 });
            n = i + 1;
        }
    }
